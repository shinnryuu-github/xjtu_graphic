...
Matrix4f Camera::projection()
{
    ...
    // projection(2, 3) = 0.0f;
    // projection(3, 3) = 1.0f;
    projection(0, 0) = float(1.0 / (aspect_ratio * std::tan(fov_y / 2.0)));
    projection(1, 1) = float(1.0 / (std::tan(fov_y / 2.0)));
    projection(2, 2) = (near_plane + far_plane) / (near_plane - far_plane);
    projection(2, 3) = float((2.0 * near_plane * far_plane) / (near_plane - far_plane));
    projection(3, 2) = -1.0;
