    ...
    if (ImGui::BeginTabItem("Layout")) {
        ...
        if (selected_object != nullptr) {
            ...
            selected_object->rotation =
                Quaternionf(cos(radians(x_angle) / 2), sin(radians(x_angle) / 2), 0.0, 0.0) *
                Quaternionf(cos(radians(y_angle) / 2), 0.0, sin(radians(y_angle) / 2), 0.0) *
                Quaternionf(cos(radians(z_angle) / 2), 0.0, 0.0, sin(radians(z_angle) / 2));
        }
        ImGui::EndTabItem();
    }
}
